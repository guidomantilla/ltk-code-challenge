...
func (h *handlers) PostEvents(gctx *gin.Context) {
	...
	logger := log.Ctx(ctx).With().Str("stage", "runtime").Str("component", "handlers").Logger()
	...
	if err != nil {
		logger.Error().Err(err).Msg("event validation failed")
	...
	if err != nil {
		logger.Error().Err(err).Msg("saving event failed")
...
func (h *handlers) GetEvents(gctx *gin.Context) {
	...
	if len(body) != 0 {
		logger.Err(err).Msg("request body is not empty")
	...
	id := gctx.Param("id")
	if len(id) == 0 {
		logger.Error().Err(err).Msg("parameter 'id' is required")
	...
	if err != nil {
		...
		if errors.Is(err, ErrEventNotFound) {
			logger.Info().Msg("event not found")
